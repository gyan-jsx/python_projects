...
def drawBox(img, bbox):
    ...
    cv2.putText(
        img, "Tracking", (75, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2
    )
...
def goal_track(img, bbox):
    ...
    # 2 stands for radius, (0, 0, 255) for color, and 5 for thihckness
...
while True:
    ...
    print(dist)  # Goal is reached if distance is less than 20 pixel points
    if dist <= 20:
    ...
    for i in range(len(xs)-1): cv2.circle(img,(xs[i],ys[i]),2,(0,0,255),5)    
